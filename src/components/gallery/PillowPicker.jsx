...
const PillowPicker = () => {
	...
	const handleAddScrapedProduct = () => {
		if (scrapedData && scrapedData.mainProduct) {
			const newMainProduct = {
				title: scrapedData.mainProduct.title,
				image: scrapedData.mainProduct.image,
				url: scrapedData.mainProduct.url,
				goesWellWith: scrapedData.mainProduct.goesWellWith,
				youMayAlsoLike: scrapedData.mainProduct.youMayAlsoLike,
			};
			setMoodboardState({
				selectedGalleryItems: [...(activeMoodboard?.selectedGalleryItems || []), newMainProduct],
			});
